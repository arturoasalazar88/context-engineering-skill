...
```
/context-refactor [--report-only] [--auto]
```
...
Read the canonical templates from `context-engineering-skill/templates/` to use as the standard:
...
These can be applied automatically without confirmation:
...
These require user review and approval:
...
These cannot be automated - provide guidance:
...
Run `/context-refactor --report-only` to re-audit.
```
...
**Templates:**
- ALWAYS read from `context-engineering-skill/templates/`
...
**Compatibility:**
- Use same token estimation as `/context-start`: lines × 10
- Use same verification checks as `/context-init` and `/context-ingest`
- Reference the same standards documented in `docs/`
- Don't conflict with other commands' functionality

**Conservative Approach:**
- Only fix clear violations of documented standards
- When in doubt, report as INFO/SUGGESTION, not error
- Respect intentional deviations (if user has >150 lines but well-structured, note it but don't force)
