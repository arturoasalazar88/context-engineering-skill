---
description: Load project context properly, report readiness status, and select a story to work on
---
...
Check if `context/workflows/` directory exists at the project root.

**If it exists:**
1. List all `.md` files in `context/workflows/`
2. For each file, read the first 20 lines
3. Check for markers: `🔒 MANDATORY` or `🔒 INVIOLABLE` (anywhere in those 20 lines)
4. Classify each workflow:
   - **Mandatory**: Contains `🔒 MANDATORY` or `🔒 INVIOLABLE` marker → read the full file now (auto-load)
   - **Available**: No marker found → note the filename for listing (do not load)
5. Count lines in each mandatory workflow for token estimation

**If it does not exist:**
Skip this step silently. No workflows to report.

### Step 4: Calculate Token Budget

For each file loaded (always-loaded context + mandatory workflows):
- Count total lines
- Estimate tokens: lines x 10 (rough approximation)

Calculate separately:
- **Static context total**: CLAUDE.md + `.claude/rules/*.md`
- **Mandatory workflows total**: All auto-loaded workflow files
- **Combined total**: Static context + mandatory workflows
...
**If mandatory workflows were found in Step 3, add:**

```
Mandatory workflows (auto-loaded):
  [filename].md:                 XX lines (~X,XXX tokens) 🔒
  [filename].md:                 XX lines (~X,XXX tokens) 🔒
  ------------------------------------------------
  Total mandatory workflows:     XX lines (~X,XXX tokens)
```

**If available (non-mandatory) workflows were found, add:**
...
### Step 7: Display Stories and Offer Selection
...
Context loaded: ~X,XXX tokens (static) [+ ~X,XXX tokens (mandatory workflows)]
Mandatory workflows: X loaded [or: None detected]
...
- This workflow replaces manual session-start procedures
...
- Mandatory workflows (🔒 MANDATORY / 🔒 INVIOLABLE) are auto-loaded to prevent rule violations
- Available workflows are listed but not loaded to keep token usage efficient
